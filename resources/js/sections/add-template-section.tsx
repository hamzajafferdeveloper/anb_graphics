...
/**
 * SvgViewer: memoized component that renders the SVG via dangerouslySetInnerHTML.
 * It is memoized so that changes to the outer transform (zoom/pan) won't re-create
 * the SVG DOM and thus won't remove event listeners attached to the SVG.
 */
...
const AddTemplateSection = ({ product }: { product: Product }) => {
    ...
    // outerRef is the transform wrapper (pan/zoom applied here)
    ...
    // keep latest selectedParts for click handlers (so we don't need to rebind)
    ...
    const handleColorChange = (partId: string, newColor: string) => {
        ...
        if (el) {
            ...
            // sync svgContent (so saving uses current markup)
    ...
    // BIND click handler once when svgContent changes (not on zoom/pan)
    useEffect(() => {
        if (!svgContent || !svgRootRef.current) return;
        ...
        const onSvgClick = (event: Event) => {
            // if panning is active, ignore clicks (avoids accidental selection on mouse release)
        ...
        // only re-run when svgContent changes
    }, [svgContent, fillMap]);
    ...
    const handleSubmit = () => {
        ...
        console.log(payload);
        // send with router.post(...) if you want
    };
    ...
    // toggle sidebar once on mount if needed
    useEffect(() => {
    ...
    // Show per-part hover color using CSS variable
    useEffect(() => {
        if (!svgRootRef.current) return;
        const svgEl = svgRootRef.current.querySelector('svg');
        if (!svgEl) return;
    ...
    // Mouse wheel zoom — keeps cursor point stable
    useEffect(() => {
        const el = outerRef.current;
        if (!el) return;

        const handleWheel = (e: WheelEvent) => {
            // only when SVG present
            if (!svgRootRef.current) return;
            ...
            // compute mouse position relative to outer element
            ...
            setPan((prev) => {
                ...
                return {
                    x: prev.x - (mx - prev.x) * (scaleFactor - 1),
                    y: prev.y - (my - prev.y) * (scaleFactor - 1),
                };
            });
                        ...
                        <>
                            <div className="flex w-full items-center justify-center p-4 lg:w-[calc(100%-10rem)]">
                                <div className="flex aspect-square max-h-[90vh] w-full items-center justify-center overflow-hidden rounded-md border">
                                    {/* Outer wrapper — transform (pan/zoom) applied here */}
                                    <div
                                        ref={outerRef}
                                        ...
                                        style={{
                                            ...
                                            touchAction: 'none',
                                            // make sure pointer events pass into child SVG
                                            pointerEvents: 'auto',
                                        }}
                                        onMouseDown={startPan}
                                        onTouchStart={startPan}
                                    >
                                        {/* SvgViewer is memoized; it will only re-render when svgContent changes */}
                                        <div
                                            ref={svgRootRef}
                                            className="h-full w-full touch-none object-contain"
                                            // we keep this wrapper as plain div; SvgViewer / inner HTML is placed here
                                        >
