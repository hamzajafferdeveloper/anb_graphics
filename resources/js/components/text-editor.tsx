...
const CustomTextEditor: React.FC<Props> = ({
    onChange,
    initialValue = '',
    placeholder,
}) => {
        ...
        toolbarButtons.forEach(({ cmd, arg }) => {
            try {
                if (cmd === 'formatBlock' && arg) {
                    if (parent?.tagName.toLowerCase() === arg.toLowerCase())
                        active.push(cmd + arg);
                } else if (
    ...
    return (
        <div className="space-y-2 rounded-md border p-2">
                    ...
                    <button
                        ...
                        className={`cursor-pointer rounded border px-2 py-1 text-xs transition ${
                            activeCommands.includes(cmd + (arg || ''))
                                ? 'border-gray-400 bg-gray-200/80 dark:border-gray-600 dark:bg-gray-700/30 dark:text-white'
                                : 'hover:bg-gray-200/80 hover:text-gray-900 dark:text-gray-300 dark:hover:bg-gray-700/30'
                        }`}
            ...
            <div
                ...
                className="editor-content prose min-h-[200px] rounded border p-3 focus:outline-none"
                style={{
                    direction: 'ltr',
                    whiteSpace: 'pre-wrap',
